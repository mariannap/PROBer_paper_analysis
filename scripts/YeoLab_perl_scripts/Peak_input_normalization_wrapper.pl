...
use FindBin;
my $samtools_path = "/home/bli/PROBer_paper_analysis/tools/samtools-1.3.1";
...
## This will not run properly if the manifest is structured differently
	...
	unless (exists $mapped_num{$clip_bam_fi_short}) {
	    my $mapped_read_num = `$samtools_path/samtools view -c -F 4 $clip_bam_fi_softlink`;
	    $mapped_num{$clip_bam_fi_short} = $mapped_read_num;
	}
	unless (exists $mapped_num{$input_bam_fi_short}) {
	    my $input_read_num = `$samtools_path/samtools view -c -F 4 $input_bam_fi_softlink`;
...
for my $line (<F>) {
	...
	for my $compare_peakfi (@desired_peak_filelist) {
		...
		if (-e $output_compressed) {
		    ...
		    system("perl $FindBin::RealBin/compress_l2foldenrpeakfi.pl $output");
		}
	    } else {
		system("perl $FindBin::RealBin/overlap_peakfi_with_bam_PE.pl $clip_bam_fi_softlink $input_bam_fi_softlink $compare_peakfi_softlink $filist_fi_mappedreadnums $output");
		system("perl $FindBin::RealBin/compress_l2foldenrpeakfi.pl $output");
	    }
