...
int main(int argc, char* argv[]) {
  if (argc < 4) {
    printf("Usage: calcMappability k input_reference_fasta_file output_file('NA' means no output) [--inspect transcript_name]\n");
    exit(-1);
  }
  ...
  for (size_t i = 1; i <= cands.size(); i++)
	...
	if (j == i || cands[q].tid != cands[j].tid) {
	  ...
	  if (numerator < denominator) {
	    clusteringInfo[cands[q].tid] += numerator;
	    if (inspect_name == names[cands[q].tid]) hasSeen = true;
	  }
	  q = j;
	}

      if (hasSeen) {
	for (size_t j = p; j < i; ++j) printf("(%s, %d)\t", names[cands[j].tid].c_str(), cands[j].pos);
	printf("\n");
      }
  ...
  for (int i = 1; i <= M; i++) 
    ...
    else clusteringInfo[i] = 1.0 - clusteringInfo[i] / effL[i];
